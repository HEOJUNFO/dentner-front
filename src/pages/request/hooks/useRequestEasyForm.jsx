...
import { postRequestSimple, putRequestSimple, postRequestOften, getRequestJson } from '@api/Request';
 ...
 * @returns
 */
export const useRequestEasyForm = () => {
  ...
  const handleAddTeethType = (p) => {
    ...
    console.log('handleAddTeethType --> ' , p)
    let v = {};
    if (dept === 'code') {
      ...
      v = {
        ...
        typeCount: (lowerCode.direct || code?.direct) ? '1' : '',
        requestTypeName: name,
        requestMiddleValue: middleCode.value,
        direct: lowerCode?.direct ? lowerCode?.direct : code?.direct
  ...
  const fetchData = async (requestDocGroupNo) => {
    const r = await getRequestJson(requestDocGroupNo);
    const dt = r.data;
    // console.log('dt', dt);
