...
const TableSortingM = ({ className, defaultValue, titleName, valueName, groupName, items = [], onChange, input = false, eventType = 'change', button = false, buttonTitle, buttonPosition }) => {
    const [visible, setVisible] = useState(false);
    let buttonItems = items?.filter((el) => el[titleName] === buttonTitle)
    let selectItems = button ? items?.filter((el) => el[titleName] !== buttonTitle) : items
    ...
    <>
      <div className="right sortingSet">
        <span className={className} style={{position: 'absolute', ...buttonPosition}}>
            {button && buttonItems?.map((el, idx) => { 
                let func = onChange;
                if (el.onChange) func = el.onChange;
                return (
                    <span>
                        <input type="radio" id={`${groupName}_${idx}_btn`} name={groupName} onChange={() => func(el[valueName])} />
        ...
        <span className="sArea">
          ...
          <span className={className}>
            <div  className={`fillter`}>
                <em className={`${visible ? 'on' : ''}`}  onClick={() => setVisible(!visible)}>{selected?.[titleName] || selectItems?.[0]?.[titleName]}</em>
                <div>
                {selectItems?.map((el, idx) => {
                    ...
                    return (
                    <span>
                        <input type="radio" id={`${groupName}_${idx}`} name={groupName} defaultChecked={defaultValue === el[valueName]} onChange={() => {func(el[valueName]); setVisible(!visible)}} />
